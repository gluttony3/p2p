...
def receive_messages():
    while True:
        try:
            ...
            chat_ready.configure(state="normal")
            chat_ready.insert(END, f"Сервер: {message}\n") 
            chat_ready.configure(state="disabled")
        except:
            pass
...
# Кнопки зміни шрифту
CTkButton(window, text="A+", width=40, command=increase_font).place(x=280, y=390)
CTkButton(window, text="A−", width=40, command=decrease_font).place(x=330, y=390)
